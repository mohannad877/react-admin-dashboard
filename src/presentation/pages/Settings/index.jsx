...
export default function Settings() {
  ...
  const tabs = [
    ...
    { id: 'security', name: 'خيارات أمان متقدمة', icon: Lock },
  ];
        ...
        <div className="flex-1 p-6 lg:p-8">
          ...
          {activeTab === 'profile' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">المعلومات الشخصية</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                <div className="space-y-2">
                  ...
                  <input type="text" defaultValue={currentUser?.displayName || 'المسؤول'} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-teal-500 focus:outline-none" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">الدور</label>
                  <input type="text" defaultValue="مدير نظام (Admin)" disabled className="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 text-slate-500 cursor-not-allowed" />
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'preferences' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">مظهر النظام واللغة</h3>
              
              {/* Theme */}
              <div className="space-y-3">
                <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-2">
                  <Monitor size={16} /> المظهر (Theme)
                </label>
                <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => !isDark && toggleTheme()} className={`flex items-center gap-3 p-4 rounded-xl border transition-all ${!isDark ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/20 ring-1 ring-teal-500' : 'border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                    ...
                    <span className="font-medium text-slate-700 dark:text-slate-300">الوضع الفاتح</span>
                  </button>
                  <button onClick={() => isDark && toggleTheme()} className={`flex items-center gap-3 p-4 rounded-xl border transition-all ${isDark ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/20 ring-1 ring-teal-500' : 'border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                    <Moon size={20} className={isDark ? 'text-teal-400' : 'text-slate-400'} />
                    <span className="font-medium text-slate-700 dark:text-slate-300">الوضع الليلي</span>
                  </button>
                </div>
              </div>
          ...
          {activeTab === 'notifications' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">إشعارات النظام</h3>
              <div className="space-y-4">
                {[
                  { title: 'تنبيهات المخزون', desc: 'إشعار عند اقتراب منتج من النفاذ', defaultChecked: true },
                  { title: 'المستخدمين الجدد', desc: 'إشعار عند تسجيل مستخدم جديد', defaultChecked: false },
                  { title: 'التقارير الأسبوعية', desc: 'إرسال ملخص أسبوعي للبريد', defaultChecked: true },
                  { title: 'تحديثات الأمان', desc: 'تنبيهات الدخول من أجهزة جديدة', defaultChecked: true },
                ].map((item, i) => (
          ...
          {activeTab === 'security' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300 flex flex-col items-center justify-center py-10 text-center">
              ...
              <p className="text-slate-500 max-w-sm">يرجى الانتقال إلى تبويب "الأمان" من القائمة الجانبية الرئيسية للحصول على تجربة إدارة أمان متكاملة (سجل الدخول، المصادقة الثنائية).</p>
            </div>
          )}
