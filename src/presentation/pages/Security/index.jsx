...
export default function Security() {
  ...
  // Mock login history
  const loginHistory = [
    { id: 1, device: 'MacBook Pro (Chrome)', location: 'نيويورك، الولايات المتحدة', ip: '192.168.1.1', time: t('rightNow'), status: 'active' },
    { id: 2, device: 'iPhone 13 (Safari)', location: 'دبي، الإمارات', ip: '10.0.0.4', time: t('yesterday1430'), status: 'success' },
    { id: 3, device: 'Windows PC (Edge)', location: 'لندن، المملكة المتحدة', ip: '172.16.0.2', time: t('threeDaysAgo'), status: 'success' },
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-10">
      ...
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{t('securityAndPrivacy')}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
            ...
            <h3 className="font-bold text-slate-900 dark:text-slate-100 mb-2">{t('password')}</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">{t('passwordDesc')}</p>
            <Button variant="outline" className="w-full">تغيير {t('password')}</Button>
            <p className="text-xs text-slate-400 mt-3 text-center">{t('lastChanged2Months')}</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">{t('twoFactorDesc')}</p>
            
            {twoFactorEnabled ? (
              <Button variant="danger" className="w-full" onClick={() => setTwoFactorEnabled(false)}>تعطيل المصادقة الثنائية</Button>
            ) : (
              <Button variant="primary" className="w-full" onClick={() => setShow2FAModal(true)}>{t('setup2FA')}</Button>
            )}
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">{t('securityLogDesc')}</p>
              </div>
              <Button variant="outline" size="sm">{t('logoutAllDevices')}</Button>
            </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-sm text-slate-500">
                        <span>{t('locationLabel')} {log.location}</span>
                        <span>IP: {log.ip}</span>
                      </div>
                    </div>
                  </div>
                  <div className="text-right whitespace-nowrap text-sm">
                    {log.status === 'failed' ? (
                      <span className="text-red-600 dark:text-red-400 font-medium flex items-center gap-1 justify-end">
                        <AlertCircle size={14} /> {t('failedAttempt')}
                      </span>
                    ) : (
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 text-center">
              <button className="text-sm font-semibold text-teal-600 dark:text-teal-400 hover:underline">{t('viewAllLogs')}</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md p-6 shadow-xl animate-in zoom-in-95 duration-200">
            <div className="text-center mb-6">
              ...
              <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100">{t('setup2FA')}</h3>
